...
from codeclm.utils.utils import collate, length_to_mask

ConditionType = tp.Tuple[torch.Tensor, torch.Tensor]  # condition, mask


def resolve_eval_string(s: str) -> int:

    pattern = r"\$\{eval:(.+?)\}"
    match = re.fullmatch(pattern, s.strip())
    if not match:
        raise ValueError(f"'{s}' geçerli bir '${{eval:...}}' formatında değil.")

    expr = match.group(1)
    try:
        # Güvenli bir ortamda değerlendirme yap
        allowed_names = {"__builtins__": None}
        result = eval(expr, allowed_names)
    except Exception as e:
        raise ValueError(f"İfade değerlendirilirken hata oluştu: {e}")

    if not isinstance(result, int):
        raise ValueError(f"Değerlendirilen ifade bir int değil: {result}")
...
class QwTokenizerConditioner(TextConditioner):
    def __init__(
        self, output_dim: int, token_path="", max_len=300, add_token_list=[]
    ):  # ""
        ...
        if type(max_len) == "str":
            new_max_len = resolve_eval_string(max_len)
        ...
        print(
            ...
            {
                self.text_tokenizer.convert_ids_to_tokens(i): i
                for i in self.struct_token_ids
            },
        )
    ...
    def forward(self, inputs: tp.Dict[str, torch.Tensor]) -> ConditionType:
        ...
        B = tokens.shape[0]
        is_sp_embed = torch.any(
            torch.stack([tokens == i for i in self.struct_token_ids], dim=-1), dim=-1
        )
        ...
        if self.max_len is not None:
            if inputs["input_ids"].shape[-1] > self.max_len:
                warnings.warn(
                    f"Max len limit ({self.max_len}) Exceed! \
                              {[self.text_tokenizer.convert_ids_to_tokens(i.tolist()) for i in tokens]} will be cut!"
                )
            tokens = self.pad_2d_tensor(tokens, self.max_len, self.pad_token_idx).to(
                self.output_proj.weight.device
            )
            mask = self.pad_2d_tensor(mask, self.max_len, 0).to(
                self.output_proj.weight.device
            )
            tp_cover_range = self.pad_2d_tensor(tp_cover_range, self.max_len, 0).to(
                self.output_proj.weight.device
            )
    ...
    def pad_2d_tensor(self, x, max_len, pad_id):
        ...
        if pad_len > 0:
            pad_tensor = torch.full(
                (batch_size, pad_len), pad_id, dtype=x.dtype, device=x.device
            )
...
class QwTextConditioner(TextConditioner):
    def __init__(self, output_dim: int, token_path="", max_len=300):  # ""
    ...
    def forward(
        self, inputs: tp.Dict[str, torch.Tensor], structure_dur=None
    ) -> ConditionType:
        """
        Add structure embeddings of {verse, chorus, bridge} to text/lyric tokens that
        belong to these structures accordingly,
        ...
        if self.max_len is not None:
            ...
            tokens = self.pad_2d_tensor(tokens, self.max_len, 151643).to(
                self.output_proj.weight.device
            )
            mask = self.pad_2d_tensor(mask, self.max_len, 0).to(
                self.output_proj.weight.device
...
class QuantizedEmbeddingConditioner(AudioConditioner):
    def __init__(
        self, dim: int, code_size: int, code_depth: int, max_len: int, **kwargs
    ):
        ...
        self.emb = nn.ModuleList(
            [
                nn.Embedding(code_size + 2, dim, padding_idx=code_size + 1)
    ...
    def forward(self, x: AudioCondition):
        ...
        if wav.shape[2] < self.max_len - 1:
            wav = F.pad(
                wav, [0, self.max_len - 1 - wav.shape[2]], value=self.vocab_size + 1
            )
        else:
            wav = wav[:, :, : self.max_len - 1]
        embeds1 = self.emb[0](wav[:, 0])
        embeds1 = torch.cat((self.EOT_emb.unsqueeze(0).repeat(B, 1, 1), embeds1), dim=1)
        embeds2 = sum(
            [self.emb[k](wav[:, k]) for k in range(1, self.code_depth)]
        )  # B,T,D
        embeds2 = torch.cat(
            (self.layer2_EOT_emb.unsqueeze(0).repeat(B, 1, 1), embeds2), dim=1
        )
        lengths = lengths + 1
        ...
        if lengths is not None:
            ...
            mask = torch.ones(
                (B, self.code_depth), device=embeds1.device, dtype=torch.int
            )
...
class ConditionerProvider(nn.Module):
    ...
    def audio_conditions(self):
        return [
            k for k, v in self.conditioners.items() if isinstance(v, AudioConditioner)
        ]
    ...
    def tokenize(self, inputs: tp.List[ConditioningAttributes]) -> tp.Dict[str, tp.Any]:
        ...
        assert set(text.keys() | audios.keys()).issubset(
    ...
    def forward(
        self, tokenized: tp.Dict[str, tp.Any], structure_dur=None
        ...
        for attribute, inputs in tokenized.items():
            if attribute == "description" and structure_dur is not None:
                condition1, condition2, mask = self.conditioners[attribute](
                    inputs, structure_dur=structure_dur
                )
            else:
...
class ConditionFuser(StreamingModule):
    ...
    FUSING_METHODS = [
        "sum",
        "prepend",
    ]  # , "cross", "input_interpolate"] (not support in this simplest version)
    ...
    def forward(
        ...
        if "offsets" in self._streaming_state:
            ...
            first_step = True
            offsets = torch.zeros(
                input1.shape[0], dtype=torch.long, device=input1.device
            )
        ...
        for fuse_op in self.fuse2cond.keys():
            ...
            if fuse_op == "sum" and len(fuse_op_conditions) > 0:
                ...
                for cond in reverse_list:
                    this_cond_1, this_cond_2, cond_mask = conditions[cond]
                    fused_input_1 = torch.cat(
                        (this_cond_1, fused_input_1), dim=1
                    )  # concat along T dim
                    fused_input_2 = torch.cat(
                        (this_cond_2, fused_input_2), dim=1
                    )  # concat along T dim
...
class ClassifierFreeGuidanceDropout(DropoutModule):
    ...
    def dropout_condition(
        self, sample: ConditioningAttributes, condition_type: str, condition: str
        ...
        if condition_type == "audio":
            ...
            sample.audio[condition] = self.get_null_wav(
                audio_cond.wav, sr=audio_cond.sample_rate[0]
            )
        else:
...
class ClassifierFreeGuidanceDropoutInference(ClassifierFreeGuidanceDropout):
    ...
    def dropout_condition_customized(
        self,
        sample: ConditioningAttributes,
        condition_type: str,
        condition: str,
        customized: list = None,
    ) -> ConditioningAttributes:
        """Utility function for nullifying an attribute inside an ConditioningAttributes object.
    ...
    def forward(
        ...
        for condition_type in condition_types:
            for sample in new_samples:
                for condition in sample.attributes[condition_type]:
                    self.dropout_condition_customized(
                        sample, condition_type, condition, customized
                    )
        return new_samples
